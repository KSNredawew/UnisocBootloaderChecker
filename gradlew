...
if [ "$cygwin" = "false" ] && [ "$nonstop" = "false" ] ; then
...
if [ "$cygwin" = "true" ] || [ "$msys" = "true" ] ; then
    ...
    # Now add the cygpath command line arguments
    for arg in "$@"; do
        if expr index "$arg" ';' >& /dev/null; then
            # Assume a variable, and leave it alone
            continue
        fi
        # Check if the argument is a number (e.g., a port number)
        if [ "$arg" -eq "$arg" ] 2>/dev/null; then
            continue
        fi
        # Convert the argument
        arg=`cygpath --path --ignore --mixed "$arg" | sed "s/$OURCYGPATTERN/##~##\1##~##/g"`
        # Roll the argument list
        set -- "$@" "$arg"
    done
    # Roll the argument list back
    shift `expr $# - 1`
    param=$1
    shift
    set -- "$param" "$@"
